...
| **Cloudflare Workers AI** | **Yes** — no card, starts immediately on any free Cloudflare account | Free-plan text models include `@cf/meta/llama-3.1-8b-instruct-fast`, `@cf/zai-org/glm-4.7-flash`, and others; embeddings include `@cf/baai/bge-base-en-v1.5`. Paid-only examples (do not use on free plan): `kimi-k2.6`, `kimi-k2.7-code`, `glm-5.2`. Deprecated 2026-05-30: `@cf/meta/llama-3-8b-instruct` and related non-`-fast` Llama 3/3.1 IDs. | 10,000 free Neurons/day (resets 00:00 UTC). This repo uses the Workers `AI` binding (see **This repository** below), not the REST account-ID URL. |
...
## This repository (Cloudflare Workers)

This project (`cloudflare-chatbot-widget`) uses **Cloudflare Workers AI bindings only** — no Gemini / OpenRouter / Groq / Hugging Face API keys are wired in code.

Live model IDs (defined in `src/index.js`):

| Role | Constant | Model ID | Notes |
|---|---|---|---|
| Chat (SSE) | `CHAT_MODEL` | `@cf/meta/llama-3.1-8b-instruct-fast` | Replaces deprecated `@cf/meta/llama-3-8b-instruct` (removed 2026-05-30). Free-plan eligible. |
| Chat fallback | `CHAT_MODEL_FALLBACK` | `@cf/zai-org/glm-4.7-flash` | Tried immediately if primary `env.AI.run` fails. Free-plan eligible. Workers AI only. |
| RAG embeddings | `EMBED_MODEL` | `@cf/baai/bge-base-en-v1.5` | 768-d; keep stable for Vectorize index compatibility. |

The provider tables and multi-provider fallback architecture elsewhere in this document are **portable reference** material. Do not implement them here unless a dedicated Agile V requirement approves external providers.

---

## Reference implementation in this repo

This section originally described a CodeBook e-commerce Express/TypeScript fallback stack at `backend/src/lib/ai/`. **That path does not exist in this Cloudflare Workers repository.** Treat the architecture pattern (registry + generic client + orchestrator) as optional guidance for other projects only.

It intentionally does **not** apply multi-provider fallback here — this Worker uses `env.AI.run(CHAT_MODEL)` / `env.AI.run(EMBED_MODEL)` only.
