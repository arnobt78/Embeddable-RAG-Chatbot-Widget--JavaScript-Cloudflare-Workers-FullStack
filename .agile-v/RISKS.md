...
| RISK-0002 | Open CORS + public `/api/chat` enables AI cost abuse | High | Partial | `CHAT_LIMITER` Rate Limiting binding 20/min/IP (DEC-0007); CORS still `*` | REQ-0012 |
| RISK-0003 | Silent RAG failures degrade answer quality unnoticed | Medium | Low (mitigated) | `console.error` + Sentry `captureErr` in `faq()` (DEC-0008) | REQ-0016 |
...
| RISK-0009 | Blind Next.js Redis/PostHog installs | Medium | Mitigated for Sentry | Sentry via DEC-0008 Workers path; Redis/PostHog still deferred | REQ-0021, DEC-0008 |
